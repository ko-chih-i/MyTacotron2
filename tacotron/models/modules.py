...
class ZoneoutLSTMCell(tf.keras.layers.LSTMCell):
    def __init__(self, units, zoneout_prob, **kwargs):
        super(ZoneoutLSTMCell, self).__init__(units, **kwargs)
        self.zoneout_prob = zoneout_prob

    def call(self, inputs, states, training=None):
        new_states = super(ZoneoutLSTMCell, self).call(inputs, states, training=training)
        if not training:
            return new_states

        # Implement zoneout logic (simplified example)
        output, [h, c] = new_states
        h = tf.where(tf.random.uniform(h.shape) < self.zoneout_prob, states[0], h)
        c = tf.where(tf.random.uniform(c.shape) < self.zoneout_prob, states[1], c)
        return output, [h, c]
